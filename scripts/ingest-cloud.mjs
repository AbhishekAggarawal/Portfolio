...
//
// Prerequisites (env vars or inline below):
...
//
// This script reads the profile data directly, generates embeddings with
// OpenAI, and pushes everything to your cloud ChromaDB instance.
// =============================================================================
...
// ── Profile data (mirrors lib/rag/profile-data.ts) ────────────────────────
const skills = [
...
// Simplified projects — real data comes from lib/portfolio.ts, hardcoded here for standalone use
const projects = [
  {
...
async function main() {
  ...
  console.log(`🧠 Embeddings: ${EMBEDDING_MODEL} (${EMBEDDING_API_URL})`);
  ...
  // 2. Generate embeddings in batches (to avoid rate limits)
